...
public class WoWUSRealms extends WoWRealm {
    public enum Name {
        Aegwynn("Aegwynn"),
     ...
     * is found, the {@link WoWUSRealms.Name#Unknown} realm is returned as opposed to null.
     *
     * @param name
     *
     * @return
    ...
    public static WoWUSRealms fromString(String name) {
        for (WoWUSRealms.Name realm : WoWUSRealms.Name.values()) {
            if (realm.name.equalsIgnoreCase(name)) {
                WoWUSRealms usRealm = new WoWUSRealms();
                usRealm.setName(realm);
                return usRealm;
            }
        }

        return new WoWUSRealms();
    }
